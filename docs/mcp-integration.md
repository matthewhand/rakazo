...
Saving in the UI reloads the API MCP client immediately. The worker picks up the new
server list on the next tool discovery (typically the next run). Env and file configs
still apply on process start.
