...
export class CompositeConnector implements ConnectorProvider {
  constructor(
    ...
    readonly mcp?: { discoverTools(context: AdapterContext): Promise<ConnectorTool[]>; execute(call: ConnectorCall, context: AdapterContext): AsyncIterable<ConnectorEvent> },
  ) {}
  ...
  async *execute(call: ConnectorCall, context: AdapterContext): AsyncIterable<ConnectorEvent> {
    ...
    if (this.mcp && call.tool.includes(".")) {
      const [serverName] = call.tool.split(".");
      if (serverName) {
        try {
...
export function createConnectorStack(
  ...
  mcpClient?: { discoverTools(context: AdapterContext): Promise<ConnectorTool[]>; execute(call: ConnectorCall, context: AdapterContext): AsyncIterable<ConnectorEvent> },
) {
